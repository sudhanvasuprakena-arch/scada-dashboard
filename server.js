...
async function buildProcurementCache() {
  ...
  try {
    ...
    const talukMonthly = await Promise.all(months.map(async m => {
      const [r] = await c.query(`SELECT s.sub_district_name AS taluk, SUM(a.quantity) AS purchase, SUM(a.kg_fat)*100/NULLIF(SUM(a.quantity),0) AS avgFat, SUM(a.kg_snf)*100/NULLIF(SUM(a.quantity),0) AS avgSnf, SUM(a.member_count) AS measurements FROM tbl_aggregation_data a JOIN tbl_dcs d ON a.dcs_code=d.dcs_code AND d.is_delete=0 JOIN tbl_sub_districts s ON d.sub_district_code=s.sub_district_code WHERE a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW() GROUP BY s.sub_district_name ORDER BY purchase DESC`, [m.start, m.end]);
      return r;
    }));
    const unionMonthly = await Promise.all(months.map(async m => {
      const [r] = await c.query(`SELECT SUM(a.quantity) AS purchase, SUM(a.kg_fat)*100/NULLIF(SUM(a.quantity),0) AS avgFat, SUM(a.kg_snf)*100/NULLIF(SUM(a.quantity),0) AS avgSnf, SUM(a.member_count) AS measurements FROM tbl_aggregation_data a WHERE a.table_name='tbl_milk_collection' AND a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW()`, [m.start, m.end]);
      const [ls] = await c.query(`SELECT SUM(a.quantity) AS localSale FROM tbl_aggregation_data a WHERE a.table_name='tbl_local_milk_sale' AND a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW()`, [m.start, m.end]);
      const [aw] = await c.query(`SELECT SUM(CASE WHEN is_quantity_auto=1 THEN quantity ELSE 0 END) AS autoWeightQty, SUM(CASE WHEN is_quantity_auto=0 THEN quantity ELSE 0 END) AS manualWeightQty FROM tbl_milk_collection WHERE collection_date BETWEEN ? AND ? AND collection_date<=NOW()`, [m.start, m.end]);
      return { ...r[0], localSale: ls[0].localSale, autoWeightQty: aw[0].autoWeightQty, manualWeightQty: aw[0].manualWeightQty };
    }));
    const [dailyRows] = await c.query(`SELECT DATE(a.collection_date) AS ds, SUM(a.quantity) AS qty, SUM(a.kg_fat)*100/NULLIF(SUM(a.quantity),0) AS fat, SUM(a.kg_snf)*100/NULLIF(SUM(a.quantity),0) AS snf FROM tbl_aggregation_data a WHERE a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW() GROUP BY DATE(a.collection_date) ORDER BY ds`, [last90[0], last90[last90.length-1]]);
    const [talukDailyRows] = await c.query(`SELECT sd.sub_district_name AS taluk, DATE(a.collection_date) AS ds, SUM(a.quantity) AS qty FROM tbl_aggregation_data a JOIN tbl_dcs d ON a.dcs_code=d.dcs_code AND d.is_delete=0 JOIN tbl_sub_districts sd ON d.sub_district_code=sd.sub_district_code WHERE a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW() GROUP BY sd.sub_district_name, DATE(a.collection_date) ORDER BY ds`, [last90[0], last90[last90.length-1]]);
    const talukDailyMap = {};
    talukDailyRows.forEach(r => { const t=r.taluk, d=r.ds.toISOString().split('T')[0]; if(!talukDailyMap[t]) talukDailyMap[t]={}; talukDailyMap[t][d]=parseFloat(r.qty)||0; });
    const talukDaily = {};
    Object.keys(talukDailyMap).forEach(t => { talukDaily[t]=last90.map(d=>talukDailyMap[t][d]||0); });
    const dmap = {};
    dailyRows.forEach(r => { dmap[r.ds.toISOString().split('T')[0]] = r; });
    const wireQty=last90.map(d=>parseFloat(dmap[d]?.qty)||0), wireFat=last90.map(d=>parseFloat(dmap[d]?.fat)||0), wireSnf=last90.map(d=>parseFloat(dmap[d]?.snf)||0);
    const weekly=[]; for(let i=0;i+7<=last90.length;i+=7) weekly.push({s:i,e:i+7,v:wireQty.slice(i,i+7).reduce((a,b)=>a+b,0)});
    ...
    const cur=months[2];
    const [mpcsRows] = await c.query(`SELECT s.sub_district_name AS taluk, d.dcs_name AS name, SUM(a.quantity) AS qty, SUM(a.kg_fat)*100/NULLIF(SUM(a.quantity),0) AS avgFat, SUM(a.quantity)/? AS perDay FROM tbl_aggregation_data a JOIN tbl_dcs d ON a.dcs_code=d.dcs_code AND d.is_delete=0 JOIN tbl_sub_districts s ON d.sub_district_code=s.sub_district_code WHERE a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW() GROUP BY a.dcs_code,d.dcs_name,s.sub_district_name ORDER BY qty DESC LIMIT 40`, [cur.days, cur.start, cur.end]);
    const [appR] = await c.query(`SELECT COUNT(DISTINCT CASE WHEN type=1 THEN code END) AS farmerApp, COUNT(DISTINCT CASE WHEN type=3 THEN code END) AS secretaryApp FROM tbl_app_activation WHERE is_active=1`);
    const talukLocalSale = await Promise.all(months.map(async m => {
      const [r] = await c.query(`SELECT s.sub_district_name AS taluk, SUM(a.quantity) AS localSale FROM tbl_aggregation_data a JOIN tbl_dcs d ON a.dcs_code=d.dcs_code AND d.is_delete=0 JOIN tbl_sub_districts s ON d.sub_district_code=s.sub_district_code WHERE a.table_name='tbl_local_milk_sale' AND a.collection_date BETWEEN ? AND ? AND a.collection_date<=NOW() GROUP BY s.sub_district_name`, [m.start, m.end]);
    ...
    const taluks=names.map(name=>{
      ...
      const purchase=bm.map(t=>parseFloat(t.purchase)||0), avgFat=bm.map(t=>parseFloat(t.avgFat)||0), avgSnf=bm.map(t=>parseFloat(t.avgSnf)||0), measurements=bm.map(t=>parseInt(t.measurements)||0);
      ...
      return { name, purchase, avgFat, avgSnf, measurements,
        autoWeightQty: purchase.map((v,i)=>{ const gp=parseFloat(unionMonthly[i]?.purchase)||0, ga=parseFloat(unionMonthly[i]?.autoWeightQty)||0; return gp?v*(ga/gp):0; }),
        manualWeightQty: purchase.map((v,i)=>{ const gp=parseFloat(unionMonthly[i]?.purchase)||0, gm=parseFloat(unionMonthly[i]?.manualWeightQty)||0; return gp?v*(gm/gp):0; }),
        localSale, sampleMilk:months.map(()=>null), netPurchase:purchase.map((v,i)=>v-(localSale[i]||0)),
        farmerApp:[0,0,0], secretaryApp:[0,0,0], chg:{daily:chgMoM,weekly:chgMoM,custom:chgMoM} };
    });
    const union={ purchase:unionMonthly.map(u=>parseFloat(u.purchase)||0), avgFat:unionMonthly.map(u=>parseFloat(u.avgFat)||0), avgSnf:unionMonthly.map(u=>parseFloat(u.avgSnf)||0), measurements:unionMonthly.map(u=>parseInt(u.measurements)||0), autoWeightQty:unionMonthly.map(u=>parseFloat(u.autoWeightQty)||0), manualWeightQty:unionMonthly.map(u=>parseFloat(u.manualWeightQty)||0), localSale:unionMonthly.map(u=>parseFloat(u.localSale)||0), sampleMilk:unionMonthly.map(()=>null), netPurchase:unionMonthly.map(u=>(parseFloat(u.purchase)||0)-(parseFloat(u.localSale)||0)), farmerApp:[appR[0].farmerApp,appR[0].farmerApp,appR[0].farmerApp], secretaryApp:[appR[0].secretaryApp,appR[0].secretaryApp,appR[0].secretaryApp] };
    const payload={ success:true, data:{ periods:months.map(m=>({label:m.label,days:m.days})), periodName:months[2].label, union, taluks, weightBuckets:[{label:'Grand Total',counts:[taluks.length,taluks.length,taluks.length]}], fatBuckets:[{label:'Grand Total',counts:[taluks.length,taluks.length,taluks.length]}], mpcs:mpcsRows.map(r=>({taluk:r.taluk,name:r.name,qty:parseFloat(r.qty)||0,avgFat:parseFloat(r.avgFat)||0,perDay:parseFloat(r.perDay)||0,chg:{daily:0,weekly:0,monthly:0,custom:0}})), wire:{ dates:last90, dailyAll:wireQty, fatAll:wireFat, snfAll:wireSnf, autoAll:wireQty.map(()=>90), weekly, monthStarts, monthDays, talukDaily, farmerAppCum:last90.map(()=>appR[0].farmerApp||0), secAppCum:last90.map(()=>appR[0].secretaryApp||0), netRatio:0.975, measRatio:0.113 } } };
    procurementCache=payload; procurementCacheTime=Date.now(); procurementCacheBuilding=false;
    console.log('Procurement cache refreshed at', new Date().toISOString());
  } catch(err) { procurementCacheBuilding=false; console.error('Cache build error:', err.message); }
  finally { if(c) await c.end(); }
}

app.get('/api/bangalore/union054/procurement', async (req, res) => {
  if(procurementCache && (Date.now()-procurementCacheTime) >= CACHE_TTL) buildProcurementCache();
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  setTimeout(()=>{ const http=require('http'); http.get(`http://127.0.0.1:${PORT}/api/bangalore/union054/procurement`,r=>{r.resume();console.log('Procurement cache warmed, status:',r.statusCode);}).on('error',e=>console.log('Cache warm error:',e.message)); }, 2000);
});
